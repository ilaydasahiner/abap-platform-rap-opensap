...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD get_instance_features.
    " is used for the instance specific dynamic feature control
    " specifies if the related feature is enabled or disabled based on the state of the instance
  ...
  METHOD acceptTravel.
    " set the overall status of travel instances to accepted

    MODIFY ENTITIES OF zi_rap_travel_is IN LOCAL MODE
      ENTITY Travel
         UPDATE
           FIELDS ( TravelStatus )
      ...
      FAILED failed
      REPORTED reported.
    " IN LOCAL MODE allows us to manipulate fields – e.g. to even change read-only fields – while skipping feature and authorization control.
  ...
  METHOD recalcTotalPrice.
    " calculates the total price by looping over all bookings,
    " collecting all flight prices (FlightPrice) and adding the booking fee (BookingFee).
    " In case of different currency codes, it also performs a currency conversion.
  ...
  METHOD rejectTravel.
    " set the overall status of travel instances to rejected
  ...
  METHOD calculateTotalPrice.
    " it is triggered anytime BookingFee and CurrencyCode fields are changed
  ...
  METHOD setInitialStatus.
    " sets the travel status to open when a new travel instance is created.
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-CustomerID IS INITIAL OR NOT line_exists( customers_db[ customer_id = travel-CustomerID ] ).
        ...
        APPEND VALUE #(  %tky        = travel-%tky
                         ...
                         %msg        = NEW zcm_rap_is(
                                           severity   = if_abap_behv_message=>severity-error
                                           textid     = zcm_rap_is=>customer_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-EndDate < travel-BeginDate.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_is(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_is=>date_interval
      ...
      ELSEIF travel-BeginDate < cl_abap_context_info=>get_system_date( ).
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_is(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_is=>begin_date_before_system_date
                                                 begindate = travel-BeginDate )
...
*   For the Travel Status we need the before-image from the database. We perform this for active (is_draft=00) as well as for drafts (is_draft=01) as we can't distinguish between edit or new drafts
    SELECT FROM zi_rap_travel_is
      FIELDS TravelUUID,TravelStatus
      FOR ALL ENTRIES IN @travels
      WHERE TravelUUID EQ @travels-TravelUUID
      ORDER BY PRIMARY KEY
      INTO TABLE @DATA(travels_before_image).


    " The introduction of draft handling has added two predefined actions to the business object. The draft determine action Prepare and the draft action Edit.
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      READ TABLE travels_before_image INTO DATA(travel_before_image) WITH KEY TravelUUID = travel-TravelUUID BINARY SEARCH.
      ...
      IF is_update_requested = abap_true.
        ...
        IF has_before_image = abap_true.
          update_granted = is_update_granted( has_before_image = has_before_image  overall_status = travel_before_image-TravelStatus ).
          IF update_granted = abap_false.
            APPEND VALUE #( %tky        = travel-%tky
                            %msg        = NEW zcm_rap_is( severity = if_abap_behv_message=>severity-error
                                                          textid   = zcm_rap_is=>unauthorized )
                          ) TO reported-travel.
          ENDIF.
      ...
      IF is_delete_requested = abap_true.
        delete_granted = is_delete_granted( has_before_image = has_before_image  overall_status = travel_before_image-TravelStatus ).
  ...
  METHOD is_create_granted.
    " checks if the create operation is allowed

    AUTHORITY-CHECK OBJECT 'ZOSTATIS'
        ID 'ZOSTATIS' DUMMY
  ...
  METHOD is_delete_granted.
    " checks if the delete operation is allowed

    IF has_before_image = abap_true.
      AUTHORITY-CHECK OBJECT 'ZOSTATIS'
        ID 'ZOSTATIS' FIELD overall_status
        ID 'ACTVT' FIELD '06'.
    ELSE.
      AUTHORITY-CHECK OBJECT 'ZOSTATIS'
        ID 'ZOSTATIS' DUMMY
  ...
  METHOD is_update_granted.
    " checks if the update operation is allowed
